...
用法：
    ...
    python version_manager.py --action backup --slug xiaomei --base-dir ./exes
...
def rollback(skill_dir: Path, target_version: str) -> bool:
    ...
    if meta_path.exists():
        ...
        for fname in ("SKILL.md", "memories.md", "persona.md", "events.json"):
            src = skill_dir / fname
            if src.exists():
...
def backup(skill_dir: Path) -> str:
    """将当前文件存档到 versions 目录"""
    meta_path = skill_dir / "meta.json"
    if not meta_path.exists():
        print("错误：找不到 meta.json，无法确定版本号", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"已备份版本 {current_version}，文件：{', '.join(backed_up)}")
